...
async function initAdmin() {
  ...
  await Promise.all([loadAdminProducts(), loadAdminBanners(), loadAdminUsers(), loadResellerDiscountSetting()]);
...
function refreshDashboardStats() {
  ...
  supabaseClient.from("orders").select("id", { count: "exact", head: true }).then(({ count }) => {
...
async function saveResellerDiscount() {
  const val = document.getElementById("resellerDiscountInput").value;
  const { error } = await supabase.from("settings").upsert({ key: "reseller_discount_percent", value: String(val) });
  if (error) alert("Gagal menyimpan: " + error.message);
  else alert("Diskon reseller diperbarui menjadi " + val + "%.");
}
...
async function saveProduct() {
  ...
  const { error } = id
    ...
    : await supabase.from("products").insert({ ...payload, active: true, is_flash_sale: false });
...
async function toggleFlashSale(id, checked) {
  const { error } = await supabase.from("products").update({ is_flash_sale: checked }).eq("id", id);
  if (error) alert("Gagal mengubah status flash sale: " + error.message);
  await loadAdminProducts();
}
async function updateFlashPrice(id, value) {
  const price = parseFloat(value);
  if (isNaN(price)) return;
  const { error } = await supabase.from("products").update({ flash_sale_price: price }).eq("id", id);
  if (error) alert("Gagal menyimpan harga flash sale: " + error.message);
  await loadAdminProducts();
}
...
async function saveBanner() {
  ...
  const { error } = await supabase.from("banners").insert({ image_url, sort_order, active: true });
  if (error) { alert("Gagal menambah banner: " + error.message); return; }
  closeModal("bannerModal");
  await loadAdminBanners();
}
